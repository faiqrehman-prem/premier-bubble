...
// Function to save prompt to server
...
function updateUITexts() {
  ...
  document.querySelector(".tab[data-tab='mcp-servers']").textContent =
    getText("mcpServers");
  document.querySelector(".tab[data-tab='kb']").textContent =
    getText("knowledgeBase");
...
function initConfigModal() {
  ...
  saveConfigBtn.addEventListener("click", async () => {
    try {
      ...
      console.log("Saved prompt to default.md");
...
// Helper function to translate specific error messages
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    const response = await fetch('/prompts/default.md');
    if (response.ok) {
      SYSTEM_PROMPT = await response.text();
      console.log("Loaded prompt from default.md");
      promptCache["default"] = SYSTEM_PROMPT;
    } else {
      console.error("Failed to load default.md prompt file");
      SYSTEM_PROMPT = "You are a specialized assistant that ONLY answers questions based on the knowledge base provided. If a question cannot be answered using the knowledge base, respond with 'I don't have information about that in my knowledge base.' Do not use any general knowledge to answer questions.";
    }
  } catch (error) {
    console.error("Error loading prompt from default.md:", error);
    SYSTEM_PROMPT = "You are a specialized assistant that ONLY answers questions based on the knowledge base provided.";
  }
  ...
  // Fix: Ensure chat is visible on load when isChatVisible is true
  if (isChatVisible) {
    chatContainer.style.display = "block";
    updateChatUI();
  }

  textButton.style.backgroundColor = "#4cd964";
});
