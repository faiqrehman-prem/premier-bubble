...
// Helper function to build redirect URL - different for local vs production
function buildLoginUrl(returnUrl: string): string {
  const isLocalhost = APP_URL.includes('localhost') || APP_URL.includes('127.0.0.1');
  
  if (isLocalhost) {
    // For localhost testing, use redirect_url parameter with specific format
    ...
    // For production, use callback approach with returnUrl
    const callbackUrl = `${APP_URL}/auth/callback?returnTo=${encodeURIComponent(returnUrl)}`;
    return `${LOGIN_PORTAL_URL}?returnUrl=${encodeURIComponent(callbackUrl)}`;
  }
}
      ...
      if (needsReVerification(session)) {
        ...
        if (required) {
          if (isAPI) {
            ...
            console.log(`[AUTH] Redirecting to login: ${buildLoginUrl(req.originalUrl)}`);
